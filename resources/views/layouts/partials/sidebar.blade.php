<div class="c-sidebar c-sidebar-dark c-sidebar-fixed c-sidebar-lg-show" id="sidebar">
    <div class="c-sidebar-brand d-lg-down-none">
        <svg id="full" viewBox="0 0 376 178" width="118" height="46" class="c-sidebar-brand-full" alt="CoreUI Logo">
            <g class="signet" style="fill: currentColor">
                <path class="cls-2" d="M128,56.1244,89,33.6077a12,12,0,0,0-12,0L38,56.1243a12.0339,12.0339,0,0,0-6,10.3924V111.55a12.0333,12.0333,0,0,0,6,10.3923L77,144.459a12,12,0,0,0,12,0l39-22.5167a12.0333,12.0333,0,0,0,6-10.3923V66.5167A12.0336,12.0336,0,0,0,128,56.1244ZM126,111.55a4,4,0,0,1-2,3.4641L85,137.5308a4,4,0,0,1-4,0L42,115.0141a4,4,0,0,1-2-3.4641V66.5167a4,4,0,0,1,2-3.4641L81,40.5359a4,4,0,0,1,4,0l39,22.5167a4,4,0,0,1,2,3.4641Z"></path>
                <path class="cls-2" d="M106.0216,102.0713h-2.866a3.9993,3.9993,0,0,0-1.9246.4935L83.95,112.05,64,100.5315V77.554L83.95,66.0354,101.239,75.49a4.0007,4.0007,0,0,0,1.9192.4905h2.8633a2,2,0,0,0,2-2V71.2691a2,2,0,0,0-1.04-1.7547L87.793,59.0188a8.0391,8.0391,0,0,0-7.8428.09L60,70.6262A8.0245,8.0245,0,0,0,56,77.5549v22.976a8,8,0,0,0,4,6.9283l19.95,11.5186a8.0429,8.0429,0,0,0,7.8433.0879l19.19-10.5312a2,2,0,0,0,1.0377-1.7533v-2.71A2,2,0,0,0,106.0216,102.0713Z"></path>
            </g>
            <g class="text" style="fill: currentColor">
                <path class="cls-2" d="M200.7446,65.4285a15.0166,15.0166,0,0,0-15,14.9995V97.7054a15,15,0,0,0,30,0V80.428A15.0166,15.0166,0,0,0,200.7446,65.4285Zm7,32.2769a7,7,0,0,1-14,0V80.428a7,7,0,0,1,14,0Z"></path>
                <path class="cls-2" d="M164.0789,73.4878a7.01,7.01,0,0,1,7.868,6.0752.9894.9894,0,0,0,.9843.865h6.0305a1.0108,1.0108,0,0,0,.9987-1.0971,15.0182,15.0182,0,0,0-15.7163-13.8837A15.2881,15.2881,0,0,0,150,80.8635V97.27a15.2881,15.2881,0,0,0,14.2441,15.4163A15.0183,15.0183,0,0,0,179.96,98.8025a1.0107,1.0107,0,0,0-.9987-1.0971h-6.0305a.9894.9894,0,0,0-.9843.865,7.01,7.01,0,0,1-7.868,6.0757A7.1643,7.1643,0,0,1,158,97.4612V80.6724A7.1639,7.1639,0,0,1,164.0789,73.4878Z"></path>
                <path class="cls-2" d="M246.9221,92.9944a12.1584,12.1584,0,0,0,7.1843-11.0771V78.2161A12.1494,12.1494,0,0,0,241.957,66.0667H225a1,1,0,0,0-1,1v44a1,1,0,0,0,1,1h6a1,1,0,0,0,1-1v-17h6.6216l7.9153,17.4138a1,1,0,0,0,.91.5862h6.591a1,1,0,0,0,.91-1.4138Zm-.8157-11.0771a4.1538,4.1538,0,0,1-4.1489,4.1494h-9.8511v-12h9.8511a4.1538,4.1538,0,0,1,4.1489,4.1494Z"></path>
                <path class="cls-2" d="M289,66.0667H263a1,1,0,0,0-1,1v44a1,1,0,0,0,1,1h26a1,1,0,0,0,1-1v-6a1,1,0,0,0-1-1H270v-12h13a1,1,0,0,0,1-1v-6a1,1,0,0,0-1-1H270v-10h19a1,1,0,0,0,1-1v-6A1,1,0,0,0,289,66.0667Z"></path>
                <path class="cls-2" d="M327,66.0667h-6a1,1,0,0,0-1,1V89.7142a7.0066,7.0066,0,1,1-14,0V67.0667a1,1,0,0,0-1-1h-6a1,1,0,0,0-1,1V89.7142a15.0031,15.0031,0,1,0,30,0V67.0667A1,1,0,0,0,327,66.0667Z"></path>
                <rect class="cls-2" x="336" y="66.0667" width="8" height="38" rx="1"></rect>
            </g>
        </svg>
        <svg id="signet" class="c-sidebar-brand-minimized" viewBox="0 0 160 160" width="46" height="46" alt="CoreUI Logo">
            <g style="fill: currentColor">
                <path class="cls-1" d="M125,47.091,86,24.5743a12,12,0,0,0-12,0L35,47.091a12.0336,12.0336,0,0,0-6,10.3923v45.0334a12.0335,12.0335,0,0,0,6,10.3923l39,22.5166a11.9993,11.9993,0,0,0,12,0l39-22.5166a12.0335,12.0335,0,0,0,6-10.3923V57.4833A12.0336,12.0336,0,0,0,125,47.091Zm-2,55.4257a4,4,0,0,1-2,3.464L82,128.4974a4,4,0,0,1-4,0L39,105.9807a4,4,0,0,1-2-3.464V57.4833a4,4,0,0,1,2-3.4641L78,31.5025a4,4,0,0,1,4,0l39,22.5167a4,4,0,0,1,2,3.4641Z"></path>
                <path class="cls-1" d="M103.0216,93.0379h-2.866a4,4,0,0,0-1.9246.4935L80.95,103.0167,61,91.4981V68.5206L80.95,57.002l17.2894,9.455a4,4,0,0,0,1.9192.4905h2.8632a2,2,0,0,0,2-2V62.2357a2,2,0,0,0-1.04-1.7547L84.793,49.9854a8.0391,8.0391,0,0,0-7.8428.09L57,61.5929A8.0243,8.0243,0,0,0,53,68.5216v22.976a8,8,0,0,0,4,6.9283l19.95,11.5185a8.0422,8.0422,0,0,0,7.8433.0879l19.19-10.5311a2,2,0,0,0,1.0378-1.7534v-2.71A2,2,0,0,0,103.0216,93.0379Z"></path>
            </g>
        </svg>
    </div>
    <ul class="c-sidebar-nav">
        <li class="c-sidebar-nav-item">
            <a class="c-sidebar-nav-link" href="{{ url('/') }}">
                <svg class="c-sidebar-nav-icon">
                    <use xlink:href="/icons/sprites/free.svg#cil-speedometer"></use>
                </svg>
                Dashboard<span class="badge badge-info">NEW</span></a>
        </li>

        <li class="c-sidebar-nav-title">Quick Menu</li>
        <li class="c-sidebar-nav-item c-sidebar-nav-dropdown">
            <a class="c-sidebar-nav-link c-sidebar-nav-dropdown-toggle" href="#">
                <svg class="c-sidebar-nav-icon">
                    <use xlink:href="/icons/sprites/free.svg#cil-book"></use>
                </svg>
                Manage Books</a>
            <ul class="c-sidebar-nav-dropdown-items">
                <li class="c-sidebar-nav-item">
                    <a class="c-sidebar-nav-link" href="{{ url('/books') }}"><span class="c-sidebar-nav-icon"></span> Books</a>
                </li>
                <li class="c-sidebar-nav-item">
                    <a class="c-sidebar-nav-link" href="{{ url('/books-categories') }}"><span class="c-sidebar-nav-icon"></span> Categories</a>
                </li>
            </ul>
        </li>
        <li class="c-sidebar-nav-item c-sidebar-nav-dropdown">
            <a class="c-sidebar-nav-link c-sidebar-nav-dropdown-toggle" href="#">
                <svg class="c-sidebar-nav-icon">
                    <use xlink:href="/icons/sprites/free.svg#cil-fax"></use>
                </svg>
                Manage Rents</a>
            <ul class="c-sidebar-nav-dropdown-items">
                <li class="c-sidebar-nav-item">
                    <a class="c-sidebar-nav-link" href="{{ url('/rents') }}"><span class="c-sidebar-nav-icon"></span> Rents</a>
                </li>
            </ul>
        </li>
        <li class="c-sidebar-nav-item c-sidebar-nav-dropdown">
            <a class="c-sidebar-nav-link c-sidebar-nav-dropdown-toggle" href="#">
                <svg class="c-sidebar-nav-icon">
                    <use xlink:href="/icons/sprites/free.svg#cil-people"></use>
                </svg>
                Manage Users</a>
            <ul class="c-sidebar-nav-dropdown-items">
                <li class="c-sidebar-nav-item">
                    <a class="c-sidebar-nav-link" href="{{ url('/authors') }}"><span class="c-sidebar-nav-icon"></span> Authors</a>
                </li>
                <li class="c-sidebar-nav-item">
                    <a class="c-sidebar-nav-link" href="{{ url('/users') }}"><span class="c-sidebar-nav-icon"></span> Users</a>
                </li>
            </ul>
        </li>

        <li class="c-sidebar-nav-title">Quick Settings</li>
        <li class="c-sidebar-nav-item">
            <a class="c-sidebar-nav-link" href="{{ url('/user/username') }}">
                <svg class="c-sidebar-nav-icon">
                    <use xlink:href="/icons/sprites/free.svg#cil-user"></use>
                </svg>
                Account Profile</a>
        </li>
        <li class="c-sidebar-nav-item">
            <a class="c-sidebar-nav-link" href="{{ url('/user/username/edit') }}">
                <svg class="c-sidebar-nav-icon">
                    <use xlink:href="/icons/sprites/free.svg#cil-cog"></use>
                </svg>
                Account Settings</a>
        </li>
        <li class="c-sidebar-nav-item">
            <a class="c-sidebar-nav-link" href="{{ url('/logout') }}" onclick="event.preventDefault(); document.getElementById('logout').submit()">
                <svg class="c-sidebar-nav-icon">
                    <use xlink:href="/icons/sprites/free.svg#cil-power-standby"></use>
                </svg>
                Logout</a>
            <form action="{{ route('logout') }}" id="logout" class="d-none" method="POST">@csrf</form>
        </li>
    </ul>
    <button class="c-sidebar-minimizer c-class-toggler" type="button" data-target="_parent" data-class="c-sidebar-minimized"></button>
</div>